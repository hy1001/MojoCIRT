...
sub welcome { ##     $authorized->get('/iEventCover/:patid')->to(controller => 'foo', action => 'welcome');
    ...
    $dbh = DBI->connect('DBI:mysql:database=cirt;host=mysql2.dipr.partners.org','','') or die $DBI::errstr;
...
sub postIEventCover {
    ...
    my $dbh = DBI->connect('DBI:mysql:database=cirt;host=mysql2.dipr.partners.org','','') or die $DBI::errstr;
